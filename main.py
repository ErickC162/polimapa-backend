...
def get_recomendaciones(email: str, db: Session = Depends(database.get_db)):
    ...
    return [{
        "datos": {"id_servicio": r["servicio"].id, "nombre_servicio": r["servicio"].nombre, "piso": r["servicio"].piso, "nombre_edificio": r["servicio"].edificio.nombre, "lat": r["servicio"].edificio.lat, "lng": r["servicio"].edificio.lng, "popularidad": r["servicio"].popularidad},
...
def get_edificios(db: Session = Depends(database.get_db)):
    eds = db.query(models.Edificio).all()
    return [{"id": e.id, "nombre": e.nombre, "lat": e.lat, "lng": e.lng, "descripcion": e.descripcion or "Edificio EPN", "servicios": [s.nombre for s in e.servicios]} for e in eds]